...
| FTS5 retrieval quality benchmark | `evaluation/retrieval_benchmark.py`, `scripts/run_retrieval_benchmark.py`, `make test-retrieval`, and `RETRIEVAL_BENCHMARK_*` configuration | Implemented with a checked-in sanitized corpus, deterministic precision@k/recall@k measurements, mean latency and peak-memory capture, and an explicit threshold gate for any later dense-retrieval pilot. |
...
The next milestone should run and record the FTS5 benchmark baseline on target hardware, then evaluate its configuration-derived decision gate. A local dense-retrieval pilot is allowed only if the measured FTS5 baseline fails a declared quality, latency, or memory threshold. In parallel, baseline configuration-gated local dispatch and the opt-in Phase 6 local-model corpus on target hardware. The completed bounded coordinator contract may later be bound to an explicitly reviewed specialist runner only if it preserves runtime-owned tool routing, immutable authority, verified-summary evidence, and zero autonomous retries. Embeddings remain a separately evaluated enhancement rather than a prerequisite.
